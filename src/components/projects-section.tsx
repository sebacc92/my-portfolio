...
const freelanceProjects = [
  ...
  },
  {
    ...
    url: "https://coach-fabian-avendano.vercel.app/",
    type: "freelance"
  }
];
  ...
  return (
    <section id="projects" class="py-20 bg-gradient-to-b from-background to-muted/20">
      <div class="container mx-auto px-4">
        ...
        <div class="mb-16">
          <h3 class="text-3xl font-bold text-center mb-8 text-foreground">
            {_`projectsPersonalTitle`}
          </h3>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {personalProjects.map((project) => (
              <div key={project.id} class="group relative overflow-hidden rounded-2xl border bg-card text-card-foreground shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                <div class="p-6">
                  <h4 class="text-xl font-semibold mb-3 text-foreground">{project.title}</h4>
                  ...
                  <div class="flex flex-wrap gap-2 mb-4">
                    {project.technologies.map((tech) => (
                      <span key={tech} class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-secondary text-secondary-foreground">
                        {tech}
                      </span>
                    ))}
                  </div>
                  ...
                  <div class="flex gap-2">
                    {project.github && (
                      <a 
                        ...
                        class="inline-flex items-center px-4 py-2 text-sm font-medium text-primary border border-primary rounded-lg hover:bg-primary hover:text-primary-foreground transition-colors"
                      >
